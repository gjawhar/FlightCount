...
local function wakeup(widget)
  ...
  if not widget.powerCycleCounted and widget.switchactive == 0 and triggerActive then
    ...
    if math.floor(os.clock()) >= widget.tallydelayStart + widget.tallyDelay then
      ...
      if widget.countPerPowerCycle then
  ...
  elseif not triggerActive then
    widget.tallydelayStart = 0
    widget.switchactive = 0

    if not widget.countPerPowerCycle then
      widget.powerCycleCounted = false
    end
  end
...
local function configure(widget)
  ...
  line = form.addLine("One count per power cycle")
  form.addBooleanField(line, form.getFieldSlots(line)[0],
    function() return widget.countPerPowerCycle end,
    function(value) widget.countPerPowerCycle = value end)
...
local function write(widget)
  ...
  storage.write("countPerPowerCycle", widget.countPerPowerCycle)
end
